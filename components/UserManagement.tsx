...
export const UserManagement: React.FC<UserManagementProps> = ({ users, usersWithRequests, onUpdatePassword, onDeleteUser }) => {
        ...
        <div className="bg-white p-6 rounded-lg shadow-lg border border-slate-200">
            ...
            <div className="space-y-3 max-h-60 overflow-y-auto pr-2">
                {userList.length > 0 ? userList.map(user => {
                    const isDeletable = !usersWithRequests.has(user.name);
                    return (
                                ...
                                {editingUserId !== user.id && (
                                    <div className="flex items-center gap-2">
                                        ...
                                        <button
                                            ...
                                            title={isDeletable ? "Delete User" : "Cannot delete: user is the requester on one or more requests."}
                                            className="p-2 text-slate-500 hover:bg-red-100 hover:text-red-700 rounded-full transition-colors disabled:text-slate-300 disabled:hover:bg-transparent disabled:cursor-not-allowed"
                                        >
